...
namespace WebOrdersModel
{
    ...
    public class UnitTestClassBase
    {
        ...
        {
            _driver.Log.Save(_testContext.ResultsDirectory + @"\WebOrdersModel TestResults", Log.Format.Html);
        }
        ...
        public void BasicTearDown()
        {
            if (_testContext.CurrentTestOutcome != UnitTestOutcome.Passed)
            {
                ...
                _testContext.AddResultFile(_testContext.ResultsDirectory + @"\WebOrdersModel TestResults\index.htm");
            }

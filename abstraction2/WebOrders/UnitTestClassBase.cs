...
namespace WebOrders
{
    ...
    public class UnitTestClassBase
    {
        ...
        {
            _driver.Log.Save(_testContext.ResultsDirectory + @"\WebOrders TestResults", Log.Format.Html);
        }
        ...
        public void BasicTearDown()
        {
            if (_testContext.CurrentTestOutcome != UnitTestOutcome.Passed)
            {
                ...
                _testContext.AddResultFile(_testContext.ResultsDirectory + @"\WebOrders TestResults\index.htm");
            }
